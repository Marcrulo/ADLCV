...
def frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    # First, ensure covariance matrices are positive semi-definite
    ...
    # Calculate the product term using the matrix square root
    ...
    frechet_distance = mean_term + trace_term
...
if __name__ == '__main__':
    ...
    classifier.eval()
    classifier.load_state_dict(torch.load('/home/joakim/semester_2/ADLCV/ex_2.2/ex6/weights/classifier/model.pth', map_location=device))

    unet_ddpm = UNet(device=device)
    unet_ddpm.eval()
    unet_ddpm.to(device)
    unet_ddpm.load_state_dict(torch.load('/home/joakim/semester_2/ADLCV/ex_2.2/ex6/weights/DDPM/model.pth', map_location=device))
    ...
    unet_ddpm_cFg.load_state_dict(torch.load('/home/joakim/semester_2/ADLCV/ex_2.2/ex6/weights/DDPM-cfg/model.pth', map_location=device))

    model = VGG()
    model.to(device)
    model.eval()
    model.load_state_dict(torch.load('/home/joakim/semester_2/ADLCV/ex_2.2/ex6/weights/vgg-sprites/model.pth', map_location=device))
