...
class Diffusion:
    ...
    def get_betas(self, schedule='linear'):
        if schedule == 'linear':
            return torch.linspace(start=self.beta_start, 
                                  ...
                                  steps=self.T) # HINT: use torch.linspace to create a linear schedule from beta_start to beta_end
    ...
    def q_sample(self, x, t):
        ...
        sqrt_alpha_bar = torch.sqrt(self.alphas_bar) # HINT: use torch.sqrt to calculate the sqrt of alphas_bar at timestep t
        sqrt_alpha_bar = sqrt_alpha_bar[:, None, None, None] # match image dimensions

        sqrt_one_minus_alpha_bar = torch.sqrt(1 - self.alphas_bar) # HINT: calculate the sqrt of 1 - alphas_bar at time step t
        sqrt_one_minus_alpha_bar = sqrt_one_minus_alpha_bar[:, None, None, None]# match image dimensions
        
        noise = torch.normal(torch.zeros_like(x),torch.ones_like(x)) # HINT: sample noise from a normal distribution. It should match the shape of x 
