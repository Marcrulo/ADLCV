...
def frechet_distance(mu1, sigma1, mu2, sigma2):
    ...
    # d2 = [mu]X - [mu]Y2 + tr([Sigma]X + [Sigma]Y - 2([Sigma]X[Sigma]Y)1/2).
    fid = np.linalg.norm(mu1-mu2, ord=2) + \
          np.trace(sigma1 + sigma2 - 2*linalg.sqrtm(linalg.sqrtm(sigma1) * sigma2 * linalg.sqrtm(sigma1)))
...
if __name__ == '__main__':
    ...
    generated_feat_noise = np.empty((len(test_loader.dataset), dims))
    ...
    for images, _ in tqdm(test_loader):
        ...
        noise = vgg_transform(noise/255.0)
        noise_features = get_features(model, noise)
        ...
        generated_feat_noise[start_idx:start_idx + original.shape[0]] = noise_features
    ...
    mu_noise, sigma_noise = feature_statistics(generated_feat_noise)
